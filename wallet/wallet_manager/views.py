...
class WalletsAfricaAPI:
    ...
    def _error_response(self, code, message):
        ...
        if status_code == "400":
            return badRequestResponse(status_code, message)
        if status_code == "401":
            return unAuthenticatedResponse(status_code, message)
        if status_code == "403":
            return unAuthorizedResponse(status_code, message)
        if status_code == "404":
            return resourceNotFoundResponse(status_code, message)
        if status_code == "409":
            return resourceConflictResponse(status_code, message)
        if status_code == "500":
            return internalServerErrorResponse(status_code, message)
    ...
    def _wallet_api_request(self, endpoint, http_method, payload={}):
        try:
            ...
            if response_status_code not in range(200, 299):
                ...
                return self._error_response(response_status_code, message)
